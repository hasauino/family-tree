...
def person_tree(req, person_id):
    ...
    all_persons = [person for level in levels for person in level if person.is_visible_to(req.user)]
...
def tree_from_to(req, from_id, to_id):
    ...
    for pk in [from_id, to_id]:
        ...
        if not matches[-1].exists():
            return render(req, "main/tree_navigation.html", {"error": _("Requested person does not exist")})
    ...
    while person != from_person:
        ...
        if person.parent is None and person != from_person:
            return render(
                req, "main/tree_navigation.html", {"error": _("Could not find a route from ancestor to child")}
            )
        levels.extend([[person for person in person.parent.children.all()]])
        person = person.parent

    levels.append([from_person])
    all_persons = [person for level in levels[::-1] for person in level if person.is_visible_to(req.user)]
    data = [p.as_node(req.user) for p in all_persons]
    links = [
        {
            "id": p.pk,
...
def save(req, orig_id, person_id):
    ...
    if req.user.is_authenticated:
        for child in req.POST["children"].split(","):
            if child not in person_children and child != "":
        ...
        for child in person.children.all():
            if child.name in req.POST["children"].split(",") and req.user not in child.editors.all():
...
def undo_choose(req):
    ...
    return render(req, "main/undo_choose.html", {"files_str": files_str, "file_names": file_names})
...
def changes(req):
    changes = [person for person in Person.objects.all() if person.access == "private"]
